...
export default function Contacts({
	...
	console.log(`current User : ${contacts}`);
	...
	}, [currentUser, contacts, propsData]);

	const changeCurrentChat = (index, contact, groupChatName) => {
		setCurrentSelected(index);
		if (contact) {
			changeChat(contact);
		} else {
			changeChat(groupChatName);
		}
	};
			...
			{currentUserImage && currentUserName && (
				<Container>
					...
					<div className="contacts">
						{groupChatName?.map((groupChatName, index) => {
							return (
								<div
									className={`contact ${
									...
									style={{ color: "white" }}
									key={groupChatName._id}
									onClick={() => changeCurrentChat(index, groupChatName)}>
